<x-app-layout >
    ...
    <div class='dark bg-[#0c0f16] pt-[98px] w-full min-h-screen items-center flex flex-col'>
        ...
        <div class="mt-8 rounded-xl shadow-md py-2 px-3 flex flex-row justify-center items-center bg-white gap-2 w-[50%] max-w-[500px]">
            <i class="fa-brands fa-searchengin rounded-2xl"></i>
            <input id="search" type="search" class="border-none flex-1 rounded-xl p-1 text-sm focus:outline-none focus:ring-0" placeholder="Search..." name="search" id="">
        </div>
  ...
  document.querySelectorAll('.btn').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            document.querySelectorAll('.btn').forEach((b) => {
                b.classList.remove('rounded-xl', 'inset-shadow-xl', "bg-white");
            });

            btn.classList.add('rounded-xl', 'inset-shadow-xl', "bg-white");
        });
    });
            ...
            const reservationHTML = `
                <div class="relative my-3 pt-3 rounded-md bg-gray-800 pr-6 shadow-white-xl py-2 px-2 flex flex-row justify-center items-start gap-4 w-[90%] max-w-[700px]">
                    ...
                    <div class="w-full h-full flex flex-col justify-start gap-1 items-start">
                        ...
                        <button class="absolute w-[83px] top-4 right-6 border-2 rounded-md shadow-md border-red-400 text-red-300 bg-transparent hover:bg-red-400 hover:text-white text-sm p-2 ${cancelBtnClass}">
                        ...
                        <div class="flex flex-row justify-between w-full">
                            ...
                            <button id="rebook" data-id="${reservation.get_room.id}" class="shadow-md rounded-md border-teal-300 border-2 bg-transparent text-teal-300 p-2 text-sm hover:bg-teal-500 hover:text-white ${rebookBtnClass}">
